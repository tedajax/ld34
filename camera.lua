...
function Camera:to_world(screen)
    local x = screen.x * self.scale + self.position.x - love.graphics.getWidth() / 2
    local y = screen.y * self.scale + self.position.y - love.graphics.getHeight() / 2
    return Vec2(x, y)
end
